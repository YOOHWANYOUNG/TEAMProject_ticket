...
<div class="center">
	<div style="margin-bottom:20px;">
    	<h2 style="text-align:left; margin-bottom:10px;">환불 안내</h2>
    ...
    <div>
		...
		<c:choose>
		    <c:when test="${not empty refund_list}">
		        <c:forEach var="refund" items="${refund_list}">
		            <c:if test="${refund.center_Type eq 'refund'}">
		                <tr align="center">
		                    <td>${refund.type_No }</td>
		                    <td>
		                        <span></span>
		                        <a class="alink" href="${contextPath}/center/view.do?center_No=${refund.center_No }">${refund.center_Title } </a>
		                    </td>
		                    <td><fmt:formatDate value="${refund.center_creDate }" /></td>
		                </tr>
		            </c:if>
		        </c:forEach>
		    </c:when>
		...
		<tr>
			<td colspan=8 class="fixed">
				<div class="page1">
	                 <c:if test="${totalRefundNum != null}">
			         	<c:choose>
			         		<c:when test="${totalRefundNum>100 }">
					      	...
					      	<c:when test="${totalRefundNum<100 }">
					      		<c:forEach var="page" begin="1" end="${totalRefundNum/10 +1}" step="1">
					      			<c:choose>
					      				<c:when test="${page==pageNum}">
					      					<a class="section_sel" href="${contextPath}/center/refund.do?section=${section}&pageNum=${page}">${page}</a>
					      				</c:when>
					      				<c:otherwise>
					      					<a class="section" href="${contextPath}/center/refund.do?section=${section}&pageNum=${page}">${page}</a>
					      				</c:otherwise>
					      			</c:choose>
					      		</c:forEach>
					      	</c:when>
