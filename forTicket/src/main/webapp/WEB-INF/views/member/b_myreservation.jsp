	...
	<table class="result">
		...
		<tr>
			<td colspan=8 class="fixed">
				<div class="page1">
	                 <c:if test="${totalBResNum != null}">
			         	<c:choose>
			         		<c:when test="${totalBResNum>100 }">
				                <c:forEach var="page" begin="1" end="10" step="1" >
							         <c:if test="${section >1 && page==1 }">
							         	<a class="section" href="${contextPath}/member/b_myreservation.do?mem_id=${member.mem_id}&section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;pre &nbsp;</a>
							         </c:if>
							         	<a class="section" href="${contextPath}/member/b_myreservation.do?mem_id=${member.mem_id}&section=${section}&pageNum=${page}">${(section-1)*10 +page } </a>
							         <c:if test="${page ==10 }">
							         	<a class="section" href="${contextPath}/member/b_myreservation.do?mem_id=${member.mem_id}&section=${section+1}&pageNum=${section*10+1}">&nbsp; next</a>
							         </c:if> 
					      		</c:forEach>
					      	</c:when>
					      	<c:when test="${totalBResNum==100 }">
					      	...
					      	<c:when test="${totalBResNum<100 }">
					      		<c:forEach var="page" begin="1" end="${totalBResNum/10 +1}" step="1">
					      			<c:choose>
					      				<c:when test="${page==pageNum}">
					      					<a class="section_sel" href="${contextPath}/member/b_myreservation.do?mem_id=${member.mem_id}&section=${section}&pageNum=${page}">${page}</a>
					      				</c:when>
					      				<c:otherwise>
					      					<a class="section" href="${contextPath}/member/b_myreservation.do?mem_id=${member.mem_id}&section=${section}&pageNum=${page}">${page}</a>
					      				</c:otherwise>
					      			</c:choose>
					      		</c:forEach>
					      	</c:when>
