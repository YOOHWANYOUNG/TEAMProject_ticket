...
<head>
	...
	<script>
		...
		function search_member(fixedSearchPeriod){	
		    ...
		    formObj.method="get";
		    formObj.action="${contextPath}/member/a_ProfileBus.do";
		    formObj.submit();
		}
		...
		<tr>
			<td>
				<select class="optionsize" name="s_search_type">
					...
					<option value="b_name">사업자명</option>
					<option value="mem_id">아이디</option>
					<option value="b_number">사업자 번호</option>
					<option value="b_phone">사업장 번호</option>
				</select>
			</td>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="list" items="${member_list}">
					<tr>
						<td class="font1">${list.b_name }</td>
						<td class="font1">${list.b_number }</td>
						<td class="font1">${list.mem_id}</td>
						<td class="font1">${list.pwd  }</td>
						<td class="font1">${list.b_phone }</td>
						...
						<td class="font1"><a href="${contextPath}/member/a_EditBus.do?mem_id=${list.mem_id }" class="reply">수정하기</a></td>
						<td class="font1"><a href="${contextPath }/admin/deleteBus.do?mem_id=${list.mem_id }" class="reply">삭제하기</a></td>
					</tr>
				</c:forEach>
			</c:otherwise>
		</c:choose>
             ...
             <td colspan=8 class="fixed">
                 <div class="page1">
	                 <c:if test="${totalBusNum != null}">
			         	<c:choose>
			         		<c:when test="${totalBusNum>100 }">
				                <c:forEach var="page" begin="1" end="10" step="1" >
							         <c:if test="${section >1 && page==1 }">
							         	<a class="section" href="${contextPath}/member/a_ProfileBus.do?section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;pre &nbsp;</a>
							         </c:if>
							         	<a class="section" href="${contextPath}/member/a_ProfileBus.do?section=${section}&pageNum=${page}">${(section-1)*10 +page } </a>
							         <c:if test="${page ==10 }">
							         	<a class="section" href="${contextPath}/member/a_ProfileBus.do?section=${section+1}&pageNum=${section*10+1}">&nbsp; next</a>
							         </c:if> 
					      		</c:forEach>
					      	</c:when>
					      	<c:when test="${totalBusNum==100 }">
					      	...
					      	<c:when test="${totalBusNum<100 }">
					      		<c:forEach var="page" begin="1" end="${totalBusNum/10 +1}" step="1">
					      			<c:choose>
					      				<c:when test="${page==pageNum}">
					      					<a class="section_sel" href="${contextPath}/member/a_ProfileBus.do?section=${section}&pageNum=${page}">${page}</a>
					      				</c:when>
					      				<c:otherwise>
					      					<a class="section" href="${contextPath}/member/a_ProfileBus.do?section=${section}&pageNum=${page}">${page}</a>
					      				</c:otherwise>
					      			</c:choose>
					      		</c:forEach>
					      	</c:when>
