...
function search_member(fixedSearchPeriod){	
    ...
    formObj.method="get";
    formObj.action="${contextPath}/event/B_listEvent.do";
    formObj.submit();
}
			...
			<table class="result">
				...
				<c:choose>
					<c:when test="${empty eventList}">
						<tr>
							<td colspan="9"><strong>조회된 이벤트 상품이 없습니다.</strong></td>
					</c:when>
					<c:otherwise>
						<c:forEach var="list" items="${eventList}">
								<tr>
									<td><a
										href="${contextPath}/event/detailEvent.do?event_no=${list.event_no}">${list.event_no }</a></td>
									<td><a
									...
									<td><a
										href="${contextPath}/event/modEventForm.do?event_no=${list.event_no }"
										class="reply">수정하기</a></td>
									<td><a
										href="${contextPath }/event/removeEvent.do?event_no=${list.event_no }"
										class="reply">삭제하기</a></td>
								</tr>
						</c:forEach>
					</c:otherwise>
				</c:choose>
